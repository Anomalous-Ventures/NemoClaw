...
PRs from autonomous agents must use the `agents/<slice-name>` branch prefix. This triggers `agent-scope-check` (`.github/workflows/agent-scope-check.yml`), which enforces the contract in `AGENT_CAPABILITIES.md` via `scripts/agent_scope_check.py`.
...
- **Pinned actions:** any workflow edits must use full-length commit SHAs (org policy). See `dco-check.yaml` / `commit-lint.yaml` for the canonical pattern.
